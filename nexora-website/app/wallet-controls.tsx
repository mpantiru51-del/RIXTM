...
export default function WalletControls() {
  ...
  function requireProvider() {
    ...
    showMessage({
      text: "Nu am găsit un wallet. Instalează MetaMask sau deschide pagina în browserul walletului.",
      tone: "error",
    });
    return null;
  }
  ...
  async function connectWallet() {
    ...
    try {
      ...
      showMessage({
        text: `Wallet conectat: ${shortAddress(selected)} · Sepolia`,
        tone: "success",
      });
    } catch (error) {
      showMessage({
        text: errorCode(error) === 4001 ? "Conectarea a fost anulată." : "Walletul nu s-a putut conecta.",
        tone: "error",
      });
    } finally {
      setBusy(null);
  ...
  async function addToken() {
    ...
    try {
      ...
      showMessage({
        text: added ? "RIXTM a fost adăugat în wallet." : "Adăugarea tokenului a fost anulată.",
        tone: added ? "success" : "error",
      });
    } catch (error) {
      showMessage({
        text: errorCode(error) === 4001 ? "Adăugarea a fost anulată." : "RIXTM nu s-a putut adăuga în wallet.",
        tone: "error",
      });
    } finally {
      setBusy(null);
  ...
  function explore() {
    document.getElementById("network-stats")?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  }
  ...
  return (
    <>
      <button className="explore-button" type="button" onClick={explore}>
        Explorare
      </button>
      ...
      {message && (
        <div className={`wallet-message ${message.tone}`} role="status" aria-live="polite">
          {message.text}
        </div>
      )}
    </>
